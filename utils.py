from configs import (
    ...
    get_ld_distillation_config,
    get_frozen_ld_distillation_config,
...
def configure_experiment(experiment: str, num_workers: int = 0,
    ...
    elif experiment in ["rkdd_distillation", "ld_distillation"]:
    ...
    if experiment == "kd_distillation":
        ...
        project_name = f"distillation-{freezed_flag}-{config.loss_config.loss}"
        model = DistillationCifarModel(config, hyperparams_config, num_workers)
    elif experiment == "ld_distillation":
        config_function = get_ld_distillation_config if is_unfrozen else get_frozen_ld_distillation_config
        config = config_function()
        project_name = f"logits-discriminatior-{freezed_flag}"
        model = LogitsDiscriminatorCifarModel(config, hyperparams_config, num_workers)
